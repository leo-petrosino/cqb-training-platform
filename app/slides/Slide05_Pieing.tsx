  ...
  return (
    <div className="h-full flex flex-col space-y-6">
      <div className="text-center space-y-2">
        ...
        <p className="text-military-300">Incremental exposure. Slice the pie, don't take the whole cake.</p>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center">
        <div className="relative w-full max-w-xl h-72">
          ...
          <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 400 288">
            <path 
              d="M 40 260 Q 80 240, 120 220 Q 160 200, 200 180" 
              fill="none" 
              stroke="#c9a227" 
              strokeWidth="2" 
              strokeDasharray="6 4"
              className={`transition-opacity duration-1000 ${step >= 2 ? 'opacity-40' : 'opacity-0'}`}
            />
          </svg>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {[
          ...
          { title: 'DON'T RUSH', desc: 'Speed comes from efficiency, not haste.' },
        ].map((tip, i) => (
